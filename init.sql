...
-- 3. Configurar sesión para datos masivos
SET SESSION cte_max_recursion_depth = 1000000;
SET SESSION max_execution_time = 0;

-- 4. Insertar 500.000 usuarios
...
SELECT
  ...
  (SELECT costoMes FROM locales WHERE id = (n % 50000) + 1) * (FLOOR(RAND() * 12) + 6) AS costoAlquiler,
  DATE_SUB(NOW(), INTERVAL FLOOR(RAND() * 365) DAY) AS fechaAlquiler,
  (n % 50000) + 1 AS localId,
  n AS usuarioId
FROM seq;

-- 7. Insertar ~2.5 millones de cuotas
